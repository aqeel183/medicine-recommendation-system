...
def get_predicted_value(patient_symptoms):
    ...
    try:
        ...
        return {"error": f"Invalid symptom: '{e.args[0]}'"}
...
def home():
    if request.method == 'POST':
        ...
        if symptoms == "Symptoms" or not symptoms.strip():
            message = "Please enter valid symptoms. Field cannot be empty or 'Symptoms'."
        ...
        # Call prediction function and handle invalid symptoms
        result = get_predicted_value(user_symptoms)

        if isinstance(result, dict) and "error" in result:
            error_msg = result["error"] + ". Please enter valid symptoms or consult a doctor."
